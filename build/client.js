...
function startServer() {
    ...
    const whenBanner = new Promise((resolve) => {
...
async function main() {
    ...
    if (cmd === "forecast") {
        ...
        const resp = await request(buildToolCall("get-forecast", { latitude, longitude }));
        ...
        const resp = await request(buildToolCall("get-alerts", { state }));
        console.log(resp?.result?.content?.[0]?.text ?? "No content");
    }
